...
RSpec.describe Web::PushNotificationJob do
  ...
  describe 'perform' do
    before do
      allow_any_instance_of(subscription.class).to receive(:contact_email).and_return(contact_email) # rubocop:disable RSpec/AnyInstance
      allow_any_instance_of(subscription.class).to receive(:vapid_key).and_return(vapid_key) # rubocop:disable RSpec/AnyInstance
