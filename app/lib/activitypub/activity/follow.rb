...
class ActivityPub::Activity::Follow < ActivityPub::Activity
  ...
  def perform
    ...
    if target_account.locked? || @account.silenced?
      LocalNotificationJob.perform_later(target_account.id, follow_request.id, 'FollowRequest', 'follow_request')
    else
      AuthorizeFollowService.new.call(@account, target_account)
      LocalNotificationJob.perform_later(target_account.id, ::Follow.find_by(account: @account, target_account: target_account).id, 'Follow', 'follow')
    end
  end
  ...
  def reject_follow_request!(target_account)
    ...
    ActivityPub::DeliveryJob.perform_later(json, target_account.id, @account.inbox_url)
  end
end
