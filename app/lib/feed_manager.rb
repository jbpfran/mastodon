...
class FeedManager
  ...
  def push_to_home(account, status, update: false)
    ...
    PushUpdateJob.perform_later(account.id, status.id, "timeline:#{account.id}", { 'update' => update }) if push_update_required?("timeline:#{account.id}")
    true
  end
  ...
  def push_to_list(list, status, update: false)
    ...
    trim(:list, list.id)
    PushUpdateJob.perform_later(list.account_id, status.id, "timeline:list:#{list.id}", { 'update' => update }) if push_update_required?("timeline:list:#{list.id}")
    true
  end
