...
class MoveJob < ApplicationJob
  ...
  def rewrite_follows!
    ...
    @source_account.passive_relationships
                   ...
                   .in_batches do |follows|
      ListAccount.where(follow: follows).in_batches.update_all(account_id: @target_account.id) # rubocop:disable Rails/SkipsModelValidations
      num_moved += follows.update_all(target_account_id: @target_account.id) # rubocop:disable Rails/SkipsModelValidations
    end
  ...
  def queue_follow_unfollows!
    ...
    @source_account.followers.local.select(:id).find_in_batches do |accounts|
      accounts.map(&:id).each do |follower_id|
        UnfollowFollowJob.perform_later(follower_id, @source_account.id, @target_account.id, bypass_locked)
      end
    rescue => e
      @deferred_error = e
    end
